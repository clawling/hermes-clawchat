...
async def test_on_message_records_inbound_complete_message():
    adapter = _make_adapter()
    frame = {
        "event": "message.send",
        "trace_id": "trace-1",
    ...
    assert adapter._store.messages == [
        {
            "platform": "hermes",
            "account_id": "default",
            "kind": "message",
...
async def test_on_message_does_not_record_inbound_control_frames():
    adapter = _make_adapter()
    frame = {
...
async def test_send_records_static_outbound_complete_message():
    adapter = _make_adapter(reply_mode="static")
    ...
    assert adapter._store.messages == [
        {
            ...
            "trace_id": adapter._connection.sent_frames[0]["trace_id"],
            "chat_id": "u1",
            "message_id": result.message_id,
            "text": "hi",
            "raw": adapter._connection.sent_frames[0],
        }
    ]
...
async def test_stream_records_only_final_outbound_message():
    adapter = _make_adapter(reply_mode="stream")
    ...
    assert len(adapter._store.messages) == 1
    row = adapter._store.messages[0]
    assert row["kind"] == "message"
    assert row["direction"] == "outbound"
    assert row["event_type"] == "message.done"
    assert row["message_id"] == result.message_id
    assert row["text"] == "hello world"
    assert row["raw"]["event"] == "message.done"


async def test_thinking_records_only_when_linked_to_final_message_id():
    adapter = _make_adapter(reply_mode="stream")

    await adapter.on_run_complete(
        chat_id="u1",
        final_text="<think>orphan</think>",
        message_id="missing",
    )
    assert adapter._store.messages == []
    ...
    result = await adapter.send(
        chat_id="u1",
        content="<think>private reasoning</think>visible",
    )
    await adapter.on_run_complete(
        chat_id="u1",
        final_text="<think>private reasoning</think>visible final",
        message_id=result.message_id,
    )

    assert [row["kind"] for row in adapter._store.messages] == ["message", "thinking"]
    thinking = adapter._store.messages[1]
    assert thinking["direction"] == "outbound"
    assert thinking["event_type"] == "message.done"
    assert thinking["message_id"] == result.message_id
    assert thinking["text"] == "private reasoning"
...
async def test_on_run_failed_records_outbound_error_message():
    adapter = _make_adapter(reply_mode="stream")
    first = await adapter.send(chat_id="u1", content="hello")
    ...
    assert adapter._store.messages == [
        {
            ...
            "trace_id": adapter._connection.sent_frames[-1]["trace_id"],
            "chat_id": "u1",
            "message_id": first.message_id,
            "text": "boom",
            "raw": adapter._connection.sent_frames[-1],
        }
    ]
