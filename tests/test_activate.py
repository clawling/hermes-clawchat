...
def test_activate_records_latest_activation_without_urls(tmp_path: Path, monkeypatch) -> None:
    hermes_home = tmp_path / ".hermes"
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
    activate_mod, _saved_env, _removed_env, _saved_configs = _load_activate_with_hermes_config_helpers(
        monkeypatch,
    ...
    monkeypatch.setattr(activate_mod, "ClawChatApiClient", FakeClient)
    monkeypatch.setattr(
        activate_mod,
        "get_clawchat_store",
        lambda: FakeStore(),
    ...
    asyncio.run(activate_mod.activate("ABC123", base_url="https://chat.example"))
    ...
    assert "base_url" not in store_calls[0]
    assert "websocket_url" not in store_calls[0]
