...
def _register_tools(ctx) -> None:
    activate_schema = {
        ...
        "description": _direct_tool_description(
            ...
            "ClawChat account, then persist credentials in Hermes config. Always use this when the user "
            "says a ClawChat activation/invite code or asks to activate, connect, bind, or log in ClawChat, "
            "for example `clawchat 的激活码是 R4E1IW`, `ClawChat激活码: R4E1IW`, or "
            "`activate clawchat R4E1IW`. Extract the code verbatim. If activation intent lacks a code, "
            "ask for the activation/invite code before calling this tool. Do not call connect-codes."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string",
                    "description": "ClawChat activation/invite code for the agent's connected ClawChat account. Extract it verbatim; for `clawchat 的激活码是 R4E1IW`, use `R4E1IW`.",
                },
                "baseUrl": {
                    "type": "string",
    ...
    ctx.register_tool(
        ...
        "clawchat",
        {
            ...
            "description": _direct_tool_description(
                "Fetch the agent's connected ClawChat account profile (user id, nickname/display name, avatar, bio). "
        ...
        "clawchat",
        {
            ...
            "parameters": {
                ...
                "properties": {
                    "userId": {
                        "type": "string",
                        "description": "Explicit target ClawChat user id (required). Do not infer this from a nickname.",
                    },
                },
                "required": ["userId"],
            },
    ...
    ctx.register_tool(
        ...
        "clawchat",
        {
            ...
            "description": _direct_tool_description(
                "List friends/contacts of the agent's connected ClawChat account, paginated (page=1, pageSize=20 by default). "
    ...
    ctx.register_tool(
        ...
        "clawchat",
        {
            ...
            "description": _direct_tool_description(
                "Update nickname/avatar_url/bio on the agent's connected ClawChat account, which mirrors the local assistant identity. "
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "avatar_url": {"type": "string", "description": "Hosted avatar image URL for the agent's connected ClawChat account"},
                    "bio": {"type": "string", "description": "New bio/self-introduction for the agent's connected ClawChat account, mirroring the local assistant identity"},
                },
            },
        },
    ...
    ctx.register_tool(
        ...
        "clawchat",
        {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "filePath": {"type": "string", "description": "Absolute local path of the avatar image to upload for the agent's connected ClawChat account"},
                },
                "required": ["filePath"],
            },
        },
    ...
    ctx.register_tool(
        ...
        "clawchat",
        {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "filePath": {"type": "string", "description": "Absolute local path of the non-avatar media/file to upload to ClawChat for a ClawChat-accessible URL"},
                },
                "required": ["filePath"],
            },
        },
