...
docker run -it --rm  -v ./tmp/hermes_data:/opt/data nousresearch/hermes-agent chat -q "Here is the connect <CODE>: $CODE; Install and configure clawchat by following the instructions url(use tool:'execute_code' to use python script to reqeust url): https://github.com/clawling/hermes-clawchat/raw/refs/heads/test/joe/install.md"
