...
- **Replay is ordinary downlink flow.** Missed messages arrive as regular downlink envelopes after `hello-ok`; legacy `offline.batch`, `offline.ack`, and `offline.done` are optional compatibility events only.
