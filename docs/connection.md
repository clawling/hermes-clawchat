...
Initialises state to `DISCONNECTED`, no supervisor, no websocket, no hello-wait future, empty structured `_send_queue`, `_flushing_send_queue=False`, empty pending ack map, empty inbound stream cache, a reconnect tracker, and a best-effort SQLite store handle from `get_clawchat_store()`.
...
1. Start a `connections` row with `platform="hermes"`, the configured `account_id`, `attempt`, `reconnect_count`, and `state="connecting"`. Token, `base_url`, and `websocket_url` are not passed to storage.
2. Open the WebSocket with headers `Authorization: Bearer <token>`, `X-Device-Id: <device_id>`, subprotocols `["clawchat.v1", f"bearer.{token}"]`, `ping_interval` and `ping_timeout` from config.
3. Transition to `HANDSHAKING`, answer `connect.challenge` with the msghub `ConnectPayload`, mark the row's `connect_sent_at`, and wait for matching `hello-ok`.
4. On any `hello-fail` while `HANDSHAKING`, log `auth_failed` (with `trace_id_match=true|false` for diagnostic) carrying `payload.reason`, set state to `AUTH_FAILED`, best-effort finish the row as `auth_failed`, close the socket, and stop reconnect attempts until credentials are refreshed. The `trace_id` match is logged but not required, because `hello-fail` is terminal and only one `connect` is in flight per session — strict matching would let any server bug that omits the echo (or any close before the match runs) hide the rejection reason behind a reconnect storm.
5. After `hello-ok`, set `READY`, mark the row's `ready_at`, record `ready_started_at`, log `handshake_ok`, `_flush_send_queue(ws)`, then `await self._read_task` (idle until the server disconnects).
6. `finally` branch: cancel read task, close ws, and finish the row as `closed`, `auth_failed`, or `disconnected` depending on the terminal path.
7. Cancel the stable-ready reset timer if the session disconnects before it fires.
