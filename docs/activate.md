...
Exchanges a ClawChat activation (invite) code for a token via `/v1/agents/connect`, persists secrets into `$HERMES_HOME/.env`, writes non-secret platform settings + streaming defaults into `$HERMES_HOME/config.yaml`, and best-effort upserts the latest plaintext activation credentials into `$HERMES_HOME/clawchat.sqlite`. The module imports `hermes_cli.config` helpers (`get_config_path`, `get_env_path`, `read_raw_config`, `save_config`, `save_env_value`, `remove_env_value`) at import time, so config/env persistence must go through the official Hermes config API. If those helpers are unavailable, activation fails instead of writing config files directly.
...
4. After config/env persistence succeeds, call `get_clawchat_store().upsert_activation(...)` with `platform="hermes"`, `account_id="default"`, the agent `user_id`, plaintext `access_token`, optional plaintext `refresh_token`, and `login_method="unknown"`.

The SQLite write is best-effort and logs only a generic persistence failure if it cannot complete. It does not pass or store `base_url` / `websocket_url`. This function does not add `ok` metadata or schedule a restart.
