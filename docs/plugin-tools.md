...
| `_recorded_tool_call(tool_name, args, account_id, fn)` | Execute a `clawchat_*` tool function and best-effort insert one `tool_calls` row with args, result/error, start/end times, and duration. Storage failures are logged generically and do not alter the handler result or exception behavior. |
...
Each handler is `async`, takes `(args: dict, **kw)`, logs `task_id`, records only the registered `clawchat_*` tool call in `$HERMES_HOME/clawchat.sqlite`, and returns a JSON string. The account/profile/media handlers delegate to `clawchat_gateway.tools`, which owns REST validation and error envelopes. If a backing function raises, the recorder stores `error=str(exc)` best-effort and re-raises so existing error behavior is unchanged.
