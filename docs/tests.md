...
- SQLite persistence changes: `tests/test_storage.py`, plus the focused integration file for the caller (`tests/test_activate.py`, `tests/test_connection.py`, `tests/test_adapter.py`, or `tests/test_plugin.py`).
...
- `test_activate_records_latest_activation_without_urls` — verifies `activate(...)` upserts plaintext access/refresh tokens to the plugin SQLite store without passing `base_url` or `websocket_url`.
...
- SQLite message persistence — records parsed complete inbound `message.send`, skips inbound control frames, skips streaming outbound `message.created` / `message.add`, records final outbound `message.reply` / `message.done`, records `message.failed` as `kind="error"`, and records linked `kind="thinking"` rows only when a final outbound message id exists.
...
- SQLite connection lifecycle persistence records `start_connection -> mark_connect_sent -> mark_connection_ready -> finish_connection`, avoids token/URL fields, and records terminal auth failure as `state="auth_failed"`.
...
- Tool-call persistence coverage verifies `clawchat_*` handlers record args, result/error, started/ended timestamps, and duration-compatible fields while preserving success serialization and exception behavior.
...
- `default_db_path` returns `$HERMES_HOME/clawchat.sqlite` with the fixed database filename.
- `json_dumps` uses deterministic key ordering and preserves Unicode.
- `ClawChatStore.initialize` creates `schema_migrations`, `clawchat_messages`, `activations`, `connections`, and `tool_calls`, and records migration `1 / initial_schema`.
- Activation upsert keeps only the latest row per `(platform, account_id)`.
- Message, connection lifecycle, and tool-call inserts write the expected schema fields and duration math.
