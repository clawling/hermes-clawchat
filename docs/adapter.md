...
|---|---|---|
| `async send` | `(chat_id, content="", reply_to=None, metadata=None, **kwargs) -> SendResult` | Suppresses tool-progress noise, filters `<think>` / tool blocks, builds fragments, then either emits a single `message.reply` (static mode — non-stream config or media-only) or `message.created` + `message.add` with the first delta. Registers an `_ActiveRun` for the new `message_id`. |
| `async edit_message` | `(chat_id, message_id, content, finalize=False, **kwargs) -> SendResult` | Resolve active run; compute delta against `run.last_text`; emit `message.add` with `sequence += 1` (skipped when delta is empty). When `finalize=True`, also `await on_run_complete(chat_id, content, message_id=...)` to emit `message.done` + `message.reply`. The trailing `**kwargs` swallows unknown flags from newer hermes-agent stream consumers (which now call `edit_message(..., finalize=...)` on every streamed edit) so unrecognised kwargs no longer raise `TypeError`. Tool-progress suppression is bypassed when `finalize=True`. Returns `success=False, error="no active run for message_id"` if the run was discarded. |
...
| `_should_suppress_tool_progress(content) -> bool` | `True` when every non-blank line matches `_TOOL_PROGRESS_LINE_RE` and tool output is hidden. |
| `async _build_fragments(content="", metadata=None, kwargs=None) -> list[dict]` | Produce `[{"kind": "text", ...}]` + any uploaded media fragments (empty text fallback if both are empty). |
