...
| `_CLAWCHAT_ACTIVATION_PROMPT` | str | Injected `channel_prompt` that tells the LLM how ClawChat activation is handled when activation intent is detected. |
...
Verifies `websockets` is importable and that `platform_config.extra` has non-empty `websocket_url` and `token`. Logs a warning and returns `False` otherwise. The v0.12+ platform registry path has equivalent requirement/config validators in the plugin entrypoint.
...
`__init__(self, platform_config)` — builds `ClawChatConfig.from_platform_config(platform_config)`, creates a `ClawChatConnection` bound to `_on_message` / `_on_state_change`, obtains a best-effort SQLite store handle from `get_clawchat_store()`, and initialises empty active-run / typing state maps.
...
| `async _on_message(frame: dict)` | Ignore `interaction.submit`, parse message downlinks via `parse_inbound_message`, skip if filtered, best-effort insert one inbound `clawchat_messages` row for complete `message.send` / `message.reply` frames, then delegate to `_handle_inbound`. The WebSocket connection should only call this for `message.send` / `message.reply`. |
| `async _handle_inbound(inbound: InboundMessage)` | Resolve `reply_preview`, download media, build `MessageEvent`, attach group-only covenant `channel_prompt` and activation prompt when applicable, then `await self.handle_message(event)`. |
| `_has_activation_intent(text) -> bool` | `True` iff `_ACTIVATION_INTENT_RE` matches. |
...
|---|---|---|
| `async send` | `(chat_id, content="", reply_to=None, metadata=None, **kwargs) -> SendResult` | Suppresses tool-progress noise according to `show_tool_progress`, filters `<think>` / raw tool blocks according to `show_*_output`, builds fragments, then either emits a single materialized `message.reply` with `wait_for_ack=True` (static mode — non-stream config or media-only/rich interaction) or fire-and-forget `message.created` + `message.add` with the first delta. Static mode best-effort inserts one outbound complete-message row after the acked send returns. Streaming mode registers an `_ActiveRun` for the new `message_id` and does not insert rows for `message.created` / `message.add`. |
...
| `async on_run_complete` | `(chat_id, final_text, message_id=None) -> None` | Flush final delta, emit `message.done`, best-effort insert one outbound complete-message row, optionally insert one linked `kind="thinking"` row when closed `<think>...</think>` content exists and the final outbound `message_id` is known, and discard the run from tracking maps. It intentionally does not emit a trailing materialized `message.reply`; static mode is the only path that emits `message.reply`. Duplicate completion for the same completed `message_id` is a no-op. Hermes v0.12+ reaches this through the registered platform adapter lifecycle. |
| `async on_run_failed` | `(chat_id, error, message_id=None) -> None` | Emit `message.failed`, discard the run, and best-effort insert one outbound `kind="error"` row for the completed failure frame. |
...
| `_remember_completed_run(message_id)` | Bounded completed-run cache used to make late edits / duplicate completion callbacks idempotent. |
...
| `_record_message(...)` | Best-effort insert into `clawchat_messages`; storage errors are logged generically and never interrupt adapter flow. |
| `_record_thinking_if_present(...)` | Extract closed `<think>...</think>` blocks and insert one outbound `kind="thinking"` row only when the final outbound `message_id` is available. |
